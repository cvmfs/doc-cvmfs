...
This allows file system content rollbacks to previously created and tagged revisions.
Also \cvmfs\ clients can explicitly mount a repository at a specific named snapshot to expose the file system content published with this tag.

Note that tag names need to be unique for each repository and are not allowed to contain spaces or spacial characters.
...
For that the -a and -m options for \texttt{cvmfs\_server publish} are used.
Publishing a \cvmfs\ revision like this will produce a new revision that is tagged as ``release-1.0'': 
\begin{verbatim}
...
Managing existing tags is done by using the \texttt{cvmfs\_server tag} command.
