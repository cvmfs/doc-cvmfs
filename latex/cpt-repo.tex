...
This script is not provided by \cvmfs\ but needs to be written by the repository owner.
The script needs to read requests from stdin and write mapped URLs to stdout, for instance:
\begin{verbatim}
...
Automatically generated catalogs contain a \texttt{.cvmfsautocatalog} file (along with the \texttt{.cvmfscatalog} file) in its root directory.
User-defined catalogs (containing only a \texttt{.cvmfscatalog} file) always remain untouched.
Hence one can mix both manual and automatically managed directory sub-trees.
...
Both \texttt{CVMFS\_AUTOCATALOGS\_MAX\_WEIGHT} and \texttt{CVMFS\_AUTOCATALOGS\_MIN\_WEIGHT} have reasonable defaults and usually do not need to be defined by the user.
...
\subsection{Syncing files into a repository with cvmfs\_rsync}

A common method of publishing into \cvmfs\ is to first install all the files into a convenient shared filesystem, mount the shared filesystem on the publishing machine, and then sync the files into the repository during a transaction.
The most common tool to do the syncing is \texttt{rsync}, but \texttt{rsync} by itself doesn't have a convenient mechanism for avoiding generated \texttt{.cvmfscatalog} and \texttt{.cvmfsautocatalog} files in the \cvmfs\ repository.
Actually the \texttt{-{}-exclude} option is good for avoiding the extra files, but the problem is that if a source directory tree is removed, then \texttt{rsync} will not remove the corresponding copy of the directory tree in the repository if it contains a catalog, because the extra file remains in the repository.
For this reason, a tool called \texttt{cvmfs\_rsync} is included in the \texttt{cvmfs-server} package.
This is a small wrapper around \texttt{rsync} that adds the \texttt{-{}-exclude} options and removes \texttt{.cvmfscatalog} and \texttt{.cvmfsautocatalog} files from a repository when the corresponding source directory is removed.
This is the usage:
