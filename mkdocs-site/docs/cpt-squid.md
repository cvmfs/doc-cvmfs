...
cache.org>]() servers as well. The forward proxies will reduce the
...
memory cache and hard disk cache. We suggest setting up two identical
Squid servers for reliability and load-balancing. Assuming the two
servers are A and B, set
...
it is better to use the list built in to [Frontier
Squid](https://twiki.cern.ch/twiki/bin/view/Frontier/InstallSquid#Restricting_the_destination)
because the list is sometimes updated with new releases.

The Squid configuration can be verified by `squid -k parse`. Before the
first service start, the cache space on the hard disk needs to be
prepared by `squid -z`. In order to make enough file descriptors
available to squid, execute `ulimit -n 8192` or some higher number prior
to starting the squid service.
