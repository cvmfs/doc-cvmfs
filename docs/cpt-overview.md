...
![A CernVM-FS client provides a virtual file system that loads data only
on access. In this example, all releases of a software package (such as
an HEP experiment framework) are hosted as a CernVM-FS repository on a
...
-   Use of the [Fuse kernel module](http://fuse.sourceforge.net) that
    comes with in-kernel caching of file attributes
...
-   Dynamic expansion of environment variables embedded in symbolic
    links
-   Support for extended attributes, such as file capabilities and
    SElinux attributes
...
virtual machine images or Docker images, software installed in CernVM-FS
does not need to be further packaged. Instead, it is distributed and
versioned file-by-file. In order to create and update a CernVM-FS
...
cryptographic hash of the corresponding catalog entry. The `stat()`
system call can be entirely served from the in-kernel file system
buffers.](_static/fuse.svg)
