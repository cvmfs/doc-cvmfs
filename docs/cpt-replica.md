...
The database is free, but the Maxmind [End User License
Agreement](https://www.maxmind.com/en/geolite2/eula/) requires that each
user of the database [sign up for an
account](https://www.maxmind.com/en/geolite2/signup/) and promise to
update the database to the latest version within 30 days of when they
issue a new version. The signup process will end with giving you a
License Key. The `cvmfs_server` `add-replica` and `snapshot` commands
will take care of automatically updating the database if you put a line
...
`CVMFS_GEO_DB_FILE` to the full path where you have installed it. If the
path is `NONE`, then no database will be required, but note that this
will break the client Geo API so only use it for testing, when the
