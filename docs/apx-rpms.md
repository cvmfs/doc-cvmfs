...
:   Contains the Fuse module and additional client tools. It has
    dependencies to at least one of the `cvmfs-config-...` packages.
...
:   Contains the CernVM-FS server tool kit for maintaining publishers
    and Stratum 1 servers.

**cvmfs-gateway**

:   The publishing gateway services are installed on a node with access
    to the authoritative storage.

**cvmfs-ducc**

:   Daemon that unpacks container images into a repository. Supposed to
    run on a publisher node.

**cvmfs-notify**

:   WebSockets frontend for used for repository update notifications.
    Supposed to be co-located with a RabbitMQ service.
...
:   Stand-alone utility to export file system trees into containers for
    HPC use cases.
