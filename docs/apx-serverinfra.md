...
`/etc/cvmfs/keys` and follows the naming convention `<fqrn>.crt` for the
certificate, `<fqrn>.key` for the repository's private key and
...
directives for `/var/spool/cvmfs/<fqrn>/rdonly` and `/cvmfs/<fqrn>` must
be placed into `/etc/fstab` for this reason. By default, CernVM-FS uses
the following entries for these mount points:
