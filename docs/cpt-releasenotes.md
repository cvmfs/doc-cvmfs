...
>     (#3929)
> -   \[client\] Fix race when using page cache tracker for chunked
>     files (#3685)
...
> -   \[server\] Do only one gc -a at a time, and remove need for check
>     -a to be run by root (#3575)
> -   \[server\] snapshot: Avoid recursion into history (#3846)
> -   \[server\] Optimize DNS lookups by cvmfs_geo.py to ignore short
>     host names (#3920)
