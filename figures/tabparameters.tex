...
%\begin{longtable}
	\begin{longtable}{lX}
		...
		\tt CVMFS\_FALLBACK\_PROXY		& List of HTTP proxies similar to \texttt{CVMFS\_HTTP\_PROXY}. The fallback proxies are added to the end of the normal proxies, and disable DIRECT connections.\\
		\tt CVMFS\_FOLLOW\_REDIRECTS		& When set to ``yes'', follow up to 4 HTTP redirects in requests.\\
		...
		\tt CVMFS\_MAX\_IPADDR\_PER\_PROXY & Limit the number of IP addresses a proxy names resolves into.  From all registered addresses, up to the limit are randomly selected.\\
		...
		\tt CVMFS\_USE\_GEOAPI			& Request order of Stratum 1 servers and fallback proxies via Geo-API.\\
