...
On an interactive machine, private mount points are for instance unaffected by an administrator unmounting all system's \cvmfs\ mount points by \texttt{cvmfs\_config umount}.
...
There are two options to mount \cvmfs\ in docker containers.
The first option is to bind mount a mounted repository as a volume into the container.
This has the advantage that the \cvmfs\ cache is shared among multiple containers.  
The second option is to mount a repository inside a container, which requires a \emph{privileged} container.

In both cases, \autofs\ should not be used.
Docker has often issues with \autofs.

\subsubsection{Bind mount from the host}
...
Start the docker container with the \texttt{-v} option to mount the \cvmfs\ repository inside, like

\begin{verbatim}
docker run -i -t -v /cvmfs/sft.cern.ch:/cvmfs/sft.cern.ch centos /bin/bash
\end{verbatim}

The \texttt{-v} option can be used multiple times with different repositories.

\subsubsection{Mount inside a container}

In order to use \texttt{mount} inside a container, the container must be started in privileged mode, like
\begin{verbatim}
    docker run --privileged -i -t centos /bin/bash
\end{verbatim}
In such a container, \cvmfs\ can be installed and used the usual way provided that \autofs\ is turned off.

\subsection{Parrot Connector to CernVM-FS}

In case \fuse\ cannot be be installed, the \parrot\footnote{\url{http://ccl.cse.nd.edu/software/parrot}} toolkit provides a means to ``mount'' \cvmfs\ on Linux in pure user space.  
\parrot\ sandboxes an application in a similar way {\scshape gdb} sandboxes an application.
But instead of debugging the application, \parrot\ transparently rewrites file system calls and can effectively provide /cvmfs to an application.
We recommend to use the latest precompiled \parrot\ from \url{http://ccl.cse.nd.edu/software/downloadfiles.php}, which has \cvmfs\ support built-in.

In order to sandbox a command \texttt{<CMD>} with options \texttt{<OPTIONS>} in parrot, use
...
Repositories that are not available by default from the builtin \texttt{<default-repositories>} list can be explicitly added to \texttt{PARROT\_CVMFS\_REPO}.
The repository name, a stratum 1 URL, and the public key of the repository need to be provided.
For instance, in order to add alice-ocdb.cern.ch and ilc.desy.de to the list of repositories, one can write
\begin{verbatim}
...
By default, \parrot\ uses a shared \cvmfs\ cache for all parrot instances of the same user stored under a temporary directory that is derived from the user id. 
In order to place the CernVM-FS cache into a different directory, use
\begin{verbatim}
...
The special sequence \texttt{@fqrn@} in the \texttt{CVMFS\_SERVER\_URL} string is replaced by fully qualified repository name (\eg atlas.cern.cn, cms.cern.ch, \dots).
...
The \texttt{auto} keyword used as a URL in \texttt{CVMFS\_PAC\_URLS} is resolved to \url{http://wpad/wpad.dat}, too, in order to be compatible with \product{Frontier}~\cite{frontier08}.
...
\begin{description}
	...
	\item [umount] The \texttt{umount} command unmounts all currently mounted \cvmfs\ repositories, which will only succeed if there are no open file handles on the repositories.
